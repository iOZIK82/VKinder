...
def handle_user_search(group_bot_vk, user, user_id):
    group_bot_vk.searching_users(user_id, user)

def handle_user_favorite_profiles(group_bot_vk, user_id):
...
def interaction_with_user(group_bot_vk, user, user_id):
    ...
    while True:
        user_answer = group_bot_vk.get_info_from_user(user_id=user_id,
                                                      message=slovar.options_messages['start'],
                                                      answers_list=['1', '2', '3'])
        if user_answer == '1':
            handle_user_search(group_bot_vk, user, user_id)
        elif user_answer == '2':
            handle_user_favorite_profiles(group_bot_vk, user_id)
        elif user_answer == '3':
            group_bot_vk.write_msg(user_id=user_id, message=slovar.options_messages['end_work'])
            return False
...
def main():
    ...
    """
    current_users = {}
    group_bot_vk = Bot_VK.BotVK(token_private.USER_TOKEN, token_private.GROUP_TOKEN)
    while True:
