...
class BotVK:
    ...
    def write_msg(self, user_id, message='empty message', attachment=''):
        ...
        return info_resp
    """
    Добавлен блок try/except, чтобы перехватывать "requests.exceptions.", который является базовым классом для большинства исключений, которые могут быть вызваны библиотекой "requests".
    """

    def process_user_response(self, user_id=False, initial_message='empty message',
                              follow_up_message='empty question message', attachment='', answers_list=[]):
        ...
        while True:
            ...
            """
            Метод "process_user_response" содержит в себе опрос get_answer и get_info от пользователя; вызывается соответсвующими аргументами 
            """
 ...
 def creating_user_class(self, user_id):
        ...
        :return:
        """
        current_user = setting_users.User(self.user_token, user_id)
        ...
        if len(empty_info_list) > 0:
            answer = self.asking_question_get_answer_from_user(user_id=user_id, message=message_user_info,
                                                               q_message=slovar.options_messages[
                    ...
                    while len(empty_info_list) > 0:
                        if parameter == 1:
                            ...
                            relation = self.get_info_from_user(user_id=user_id,
                                                               message=slovar.options_messages['enter_relation'],
    ...
    def searching_users(self, user_id, user_instance):
        ...
        :return:
        """
        new_users_search = search_users.UsersSearch(self.user_token, user_id, user_instance.age, user_instance.sex,
            ...
            for user_number in search_results['response']['items']:
                ...
                if 'response' in photos_info.keys():
                    ...
                    if user_answer == '1':
                        ...
                        self.write_msg(user_id=user_id, message=slovar.options_messages['users_end_search'])
                        return False
                    session.commit()
        self.write_msg(user_id=user_id, message=slovar.options_messages['end_search_list'])
        return True
