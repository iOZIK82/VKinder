...
class UsersSearch:
    ...
    def get_search_results(self, offset=0):
        params = self.__calculating_search_parameters()
        info_resp = ""
        try:
            info_resp = get(
            ...
            if info_resp.json()["error"] and info_resp.json()["error"]["error_code"] == 2:
                return {"error": "Приложение выключено."}
            elif info_resp.json()["error"] and info_resp.json()["error"]["error_code"] == 3:
                return {"error": "Передан неизвестный метод."}
    ...
    def __calculating_search_parameters(self):
        if self.age == 18:
            age_from = 18
        else:
            age_from = self.age - 1
